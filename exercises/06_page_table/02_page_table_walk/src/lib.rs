...
impl SingleLevelPageTable {
    ...
    pub fn map(&mut self, vpn: usize, ppn: u32, flags: u8) {
        self.entries[vpn] = Some(PageTableEntry { ppn: ppn, flags: flags });
    }
    ...
    pub fn translate(&self, va: u32, is_write: bool) -> TranslateResult {
        ...
        match self.entries[vpn] {
            Some(entry) => {
                ...
                let ppn = entry.ppn;
                if flags & PTE_VALID == 0 {return TranslateResult::PageFault;}
                if is_write && flags & PTE_WRITE == 0 {return TranslateResult::PermissionDenied;}
